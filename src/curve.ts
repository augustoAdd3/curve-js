...
import tricryptoFactoryABI from "./constants/abis/factory-tricrypto.json";
import {
  POOLS_DATA_ETHEREUM,
...
import {
  ...
  ycTokensFantom,
  aTokensFantom,
} from "./constants/coins/fantom.js";
import {
  COINS_AVALANCHE,
  cTokensAvalanche,
  yTokensAvalanche,
...
import {
  ...
  ycTokensAurora,
  aTokensAurora,
} from "./constants/coins/aurora.js";
import {
  COINS_KAVA,
  cTokensKava,
  yTokensKava,
...
class Curve implements ICurve {
  ...
  async init(
    ...
    if (providerType.toLowerCase() === "JsonRpc".toLowerCase()) {
      providerSettings = providerSettings as {
        url: string;
        privateKey: string;
      };
      ...
      if (providerSettings.privateKey) {
        ...
        !providerSettings.url?.startsWith("https://rpc.gnosischain.com")
      ) {
        try {
      ...
      this.provider = new ethers.BrowserProvider(
        providerSettings.externalProvider
      );
      this.signer = await this.provider.getSigner();
      // Infura provider
    ...
    this.chainId =
      Number(network.chainId) === 133 || Number(network.chainId) === 31337
        ? 1
        : (Number(network.chainId) as IChainId);
    ...
    this.constants.ALIASES.anycall =
    ...
    if (this.chainId === 1)
      this.constants.LLAMMAS_DATA =
        NETWORK_CONSTANTS[this.chainId].LLAMMAS_DATA;
    ...
    this.constants.DECIMALS = extractDecimals({
    ...
    this.setContract(
      this.constants.ALIASES.address_provider,
      addressProviderABI
    );

    const addressProviderContract =
      this.contracts[this.constants.ALIASES.address_provider].contract;
    this.constants.ALIASES.registry_exchange = (
      (await addressProviderContract.get_address(
        2,
    ...
    this.setContract(
      this.constants.ALIASES.gauge_controller,
      gaugeControllerABI
    );
    ...
    if (this.chainId === 137) {
      this.setContract(
        this.constants.ALIASES.deposit_and_stake,
        depositAndStake6CoinsABI
      );
    } else {
      this.setContract(
        this.constants.ALIASES.deposit_and_stake,
    ...
    if (this.chainId !== 1313161554) {
      ...
      this.constants.ALIASES.factory_admin = (
        (await factoryContract.admin(this.constantOptions)) as string
      ).toLowerCase();
    ...
    this.setContract(
      this.constants.ALIASES.voting_escrow_oracle,
      this.chainId === 1 ? votingEscrowOracleEthABI : votingEscrowOracleABI
    );
  }
  ...
  async _filterHiddenPools(pools: IDict<IPoolData>): Promise<IDict<IPoolData>> {
    const hiddenPools =
      (await _getHiddenPools())[this.constants.NETWORK_NAME] || [];
    // @ts-ignore
    return Object.fromEntries(
      Object.entries(pools).filter(([id]) => !hiddenPools.includes(id))
    );
  }
  ...
  fetchFactoryPools = async (useApi = true): Promise<void> => {
    ...
    if (useApi) {
      this.constants.FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, "factory")
      );
    } else {
      this.constants.FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolData.call(this)
      );
    }
    this.constants.FACTORY_POOLS_DATA = await this._filterHiddenPools(
  ...
  fetchCrvusdFactoryPools = async (useApi = true): Promise<void> => {
    ...
    if (useApi) {
      this.constants.CRVUSD_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, "factory-crvusd")
      );
    } else {
      this.constants.CRVUSD_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolData.call(
          this,
          0,
          undefined,
          this.constants.ALIASES.crvusd_factory
        )
      );
    }
    this.constants.CRVUSD_FACTORY_POOLS_DATA = await this._filterHiddenPools(
  ...
  fetchCryptoFactoryPools = async (useApi = true): Promise<void> => {
    ...
    if (useApi) {
      this.constants.CRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, "factory-crypto")
      );
    } else {
      this.constants.CRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getCryptoFactoryPoolData.call(this)
      );
    }
    this.constants.CRYPTO_FACTORY_POOLS_DATA = await this._filterHiddenPools(
  ...
  fetchTricryptoFactoryPools = async (useApi = true): Promise<void> => {
    ...
    if (useApi) {
      this.constants.TRICRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, "factory-tricrypto")
      );
    } else {
      this.constants.TRICRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getTricryptoFactoryPoolData.call(this)
      );
    }
    this.constants.TRICRYPTO_FACTORY_POOLS_DATA = await this._filterHiddenPools(
  ...
  fetchNewFactoryPools = async (): Promise<string[]> => {
    ...
    const lastPoolIdx = Number(
      currentPoolIds[currentPoolIds.length - 1].split("-")[2]
    );
    const poolData = lowerCasePoolDataAddresses(
      await getFactoryPoolData.call(this, lastPoolIdx + 1)
    );
    this.constants.FACTORY_POOLS_DATA = {
      ...this.constants.FACTORY_POOLS_DATA,
      ...poolData,
  ...
  fetchNewCryptoFactoryPools = async (): Promise<string[]> => {
    ...
    const currentPoolIds = Object.keys(
      this.constants.CRYPTO_FACTORY_POOLS_DATA
    );
    const lastPoolIdx = Number(
      currentPoolIds[currentPoolIds.length - 1].split("-")[2]
    ...
    const poolData = lowerCasePoolDataAddresses(
      await getCryptoFactoryPoolData.call(this, lastPoolIdx + 1)
    );
    this.constants.CRYPTO_FACTORY_POOLS_DATA = {
      ...this.constants.CRYPTO_FACTORY_POOLS_DATA,
      ...poolData,
  ...
  fetchNewTricryptoFactoryPools = async (): Promise<string[]> => {
    ...
    const poolData = lowerCasePoolDataAddresses(
      await getTricryptoFactoryPoolData.call(this, lastPoolIdx + 1)
    );
    this.constants.TRICRYPTO_FACTORY_POOLS_DATA = {
      ...this.constants.TRICRYPTO_FACTORY_POOLS_DATA,
      ...poolData,
  ...
  fetchRecentlyDeployedFactoryPool = async (
    poolAddress: string
  ): Promise<string> => {
    if (this.chainId === 1313161554) return "";

    const poolData = lowerCasePoolDataAddresses(
      await getFactoryPoolData.call(this, 0, poolAddress)
    );
    this.constants.FACTORY_POOLS_DATA = {
      ...this.constants.FACTORY_POOLS_DATA,
      ...poolData,
  ...
  fetchRecentlyDeployedCryptoFactoryPool = async (
    poolAddress: string
  ): Promise<string> => {
    if (![1, 137, 250].includes(this.chainId)) return "";

    const poolData = lowerCasePoolDataAddresses(
      await getCryptoFactoryPoolData.call(this, 0, poolAddress)
    );
    this.constants.CRYPTO_FACTORY_POOLS_DATA = {
      ...this.constants.CRYPTO_FACTORY_POOLS_DATA,
      ...poolData,
  ...
  fetchRecentlyDeployedTricryptoFactoryPool = async (
    poolAddress: string
  ): Promise<string> => {
    if (![1].includes(this.chainId)) return "";

    const poolData = lowerCasePoolDataAddresses(
      await getTricryptoFactoryPoolData.call(this, 0, poolAddress)
    );
    this.constants.TRICRYPTO_FACTORY_POOLS_DATA = {
      ...this.constants.TRICRYPTO_FACTORY_POOLS_DATA,
      ...poolData,
  ...
  getFactoryPoolList = (): string[] =>
    Object.keys(this.constants.FACTORY_POOLS_DATA);
  ...
  async updateFeeData(): Promise<void> {
    ...
    if (
      ...
      this.options.gasPrice =
        this.feeData.gasPrice !== undefined
