...
const _userLpBalanceCache: IDict<IDict<{ _lpBalance: bigint; time: number }>> =
  {};
const _isUserLpBalanceCacheExpired = (address: string, poolId: string) =>
  (_userLpBalanceCache[address]?.[poolId]?.time || 0) + 600000 < Date.now();

const _getUserLpBalances = async (
  pools: string[],
  address: string,
  useCache: boolean
): Promise<bigint[]> => {
  const poolsToFetch: string[] = useCache
    ? pools.filter((poolId) =>
        _isUserLpBalanceCacheExpired(address as string, poolId)
      )
    : pools;
  if (poolsToFetch.length > 0) {
    ...
    for (const poolId of poolsToFetch) {
      ...
      calls.push(
        curve.contracts[pool.lpToken].multicallContract.balanceOf(address)
      );
      if (pool.gauge !== curve.constants.ZERO_ADDRESS)
        calls.push(
          curve.contracts[pool.gauge].multicallContract.balanceOf(address)
        );
    }
    ...
    for (const poolId of poolsToFetch) {
      ...
      if (pool.gauge !== curve.constants.ZERO_ADDRESS)
        _balance = _balance + (_rawBalances.shift() as bigint);

      if (!_userLpBalanceCache[address]) _userLpBalanceCache[address] = {};
      _userLpBalanceCache[address][poolId] = {
        _lpBalance: _balance,
        time: Date.now(),
      };
    }
  ...
  for (const poolId of pools) {
    _lpBalances.push(
      _userLpBalanceCache[address]?.[poolId]._lpBalance as bigint
    );
  }
...
export const getUserLiquidityUSD = async (
  pools: string[],
  address = curve.signerAddress
): Promise<string[]> => {
...
const _userClaimableCache: IDict<
  IDict<{
    rewards: { token: string; symbol: string; amount: string }[];
    time: number;
  }>
> = {};
const _isUserClaimableCacheExpired = (address: string, poolId: string) =>
  (_userClaimableCache[address]?.[poolId]?.time || 0) + 600000 < Date.now();

const _getUserClaimable = async (
  pools: string[],
  address: string,
  useCache: boolean
  ...
  const poolsToFetch: string[] = useCache
    ? pools.filter((poolId) =>
        _isUserClaimableCacheExpired(address as string, poolId)
      )
    : pools;
  ...
  if (poolsToFetch.length > 0) {
    ...
    for (const poolId of poolsToFetch) {
      ...
      if (
        curve.chainId === 2222 ||
        pool.gauge === curve.constants.ZERO_ADDRESS
      ) {
        // TODO remove this for Kava
    ...
    for (const poolId of poolsToFetch) {
      ...
      if ("reward_tokens(uint256)" in gaugeContract) {
        // gauge_v2, gauge_v3, gauge_v4, gauge_v5, gauge_factory, gauge_rewards_only, gauge_child
        rewardCount.push(8);
    ...
    for (let i = 0; i < poolsToFetch.length; i++) {
      ...
      if (rewardCount[i] !== -1) {
        // no_gauge, gauge, gauge_v2, gauge_v3, gauge_v4, gauge_v5, gauge_factory, gauge_rewards_only, gauge_child
        ...
        const rewardContract =
          curve.contracts[pool.sRewardContract as string].contract;
        const rewardMulticallContract =
          curve.contracts[pool.sRewardContract as string].multicallContract;
    ...
    const rawRewardTokens: string[] = (
      (await curve.multicallProvider.all(rewardTokenCalls)) as string[]
    ).map((t) => t.toLowerCase());
    ...
    for (let i = 0; i < poolsToFetch.length; i++) {
      ...
      for (let j = 0; j < rewardCount[i]; j++) {
        ...
        if (
          rewardAddress ===
          "0xf228ec3476318aCB4E719D2b290bb2ef8B34DFfA".toLowerCase()
        )
          continue;
    ...
    for (let i = 0; i < poolsToFetch.length; i++) {
      ...
      const gaugeMulticallContract =
        curve.contracts[pool.gauge].multicallContract;
      ...
      for (const token of rewardTokens[poolId]) {
        ...
        if ("claimable_reward(address,address)" in gaugeContract) {
          rewardInfoCalls.push(
            gaugeMulticallContract.claimable_reward(address, token)
          );
        } else if ("claimable_reward(address)" in gaugeContract) {
          // Synthetix Gauge
          rewardInfoCalls.push(
            gaugeMulticallContract.claimable_reward(address),
            gaugeMulticallContract.claimed_rewards_for(address)
          );
        }
      }
    }
    ...
    for (let i = 0; i < poolsToFetch.length; i++) {
      ...
      if (hasCrvReward[i]) {
        ...
        if (Number(amount) > 0)
          _userClaimableCache[address][poolId].rewards.push({
            token,
            symbol,
            amount,
          });
...
const _getUserClaimableUseApi = async (
  pools: string[],
  address: string,
  useCache: boolean
): Promise<{ token: string; symbol: string; amount: string }[][]> => {
  ...
  if (poolsToFetch.length > 0) {
    ...
    for (let i = 0; i < poolsToFetch.length; i++) {
      ...
      for (const r of rewardTokens[poolId]) {
        ...
        if ("claimable_reward(address,address)" in gaugeContract) {
          rewardInfoCalls.push(
            gaugeMulticallContract.claimable_reward(address, r.token)
          );
        } else if ("claimable_reward(address)" in gaugeContract) {
          // Synthetix Gauge
          rewardInfoCalls.push(
...
export const getUserPoolListByClaimable = async (
  address = curve.signerAddress
): Promise<string[]> => {
  const pools = curve.getPoolList();
...
export const getUserClaimable = async (
  ...
  { token: string; symbol: string; amount: string; price: number }[][]
> => {
...
export const getUserPoolList = async (
  address = curve.signerAddress,
  useApi = true
): Promise<string[]> => {
  const pools = curve.getPoolList();
  const [_lpBalances, _claimable] = await Promise.all([
    ...
    useApi
      ? _getUserClaimableUseApi(pools, address, false)
...
export const _getAmplificationCoefficientsFromApi = async (): Promise<
  IDict<number>
> => {
  ...
  for (const extendedPoolData of allTypesExtendedPoolData) {
    for (const pool of extendedPoolData.poolData) {
      amplificationCoefficientDict[pool.address] = Number(
