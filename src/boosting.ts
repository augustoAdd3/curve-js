...
export const getCrv = async (
  ...
  const rawBalances = await _getBalances(
    [curve.constants.ALIASES.crv],
    addresses
  );
...
export const getLockedAmountAndUnlockTime = async (
  ...
  | IDict<{ lockedAmount: string; unlockTime: number }>
...
> => {
  ...
  const response: (string | number)[][] = (
    (await curve.multicallProvider.all(contractCalls)) as bigint[][]
  ).map((value: bigint[]) => [
    curve.formatUnits(value[0]),
    Number(curve.formatUnits(value[1], 0)) * 1000,
  ]);
...
export const getVeCrv = async (
  ...
  const response: string[] = (
    (await curve.multicallProvider.all(contractCalls)) as bigint[]
  ).map((value: bigint) => curve.formatUnits(value));
...
export const isApproved = async (amount: number | string): Promise<boolean> => {
  return await hasAllowance(
    ...
    [amount],
    curve.signerAddress,
    curve.constants.ALIASES.voting_escrow
  );
};

export const approveEstimateGas = async (
...
export const createLockEstimateGas = async (
  ...
  return Number(
    await curve.contracts[
      curve.constants.ALIASES.voting_escrow
    ].contract.create_lock.estimateGas(
      _amount,
      unlockTime,
...
export const createLock = async (
  ...
  const contract =
    curve.contracts[curve.constants.ALIASES.voting_escrow].contract;

  await curve.updateFeeData();
  const gasLimit = mulBy1_3(
    await contract.create_lock.estimateGas(
      _amount,
      unlockTime,
      curve.constantOptions
    )
...
export const increaseAmountEstimateGas = async (
  ...
  return Number(
    await contract.increase_amount.estimateGas(_amount, curve.constantOptions)
  );
};
...
export const increaseUnlockTimeEstimateGas = async (
  ...
  return Number(
    await contract.increase_unlock_time.estimateGas(
      newUnlockTime,
      curve.constantOptions
    )
  );
...
export const withdrawLockedCrv = async (): Promise<string> => {
  ...
  await curve.updateFeeData();
  const gasLimit = mulBy1_3(
    await contract.withdraw.estimateGas(curve.constantOptions)
  );
...
export const claimableFees = async (address = ""): Promise<string> => {
  ...
  const contract = new Contract(
    curve.constants.ALIASES.fee_distributor,
    feeDistributorViewABI,
    curve.provider
  );
  return curve.formatUnits(
    await contract.claim(address, curve.constantOptions)
  );
};
...
export const claimFeesEstimateGas = async (address = ""): Promise<number> => {
  address = address || curve.signerAddress;
  const contract =
    curve.contracts[curve.constants.ALIASES.fee_distributor].contract;

  return Number(
    await contract.claim.estimateGas(address, curve.constantOptions)
  );
};
...
export const lastBlockhash = async (): Promise<number> => {
  ...
  const veOracleContract =
    curve.contracts[curve.constants.ALIASES.voting_escrow_oracle].contract;

  return Number(
    await veOracleContract.last_eth_block_number(curve.constantOptions)
  );
};
...
export const getAnycallBalance = async (): Promise<string> => {
  ...
  const anycallContract =
    curve.contracts[curve.constants.ALIASES.anycall].contract;
  const _balance = await anycallContract.executionBudget(
    curve.constants.ALIASES.voting_escrow_oracle,
    curve.constantOptions
  );
...
const DEFAULT_AMOUNT =
  curve.chainId === 42161 || curve.chainId === 10 ? 0.00001 : 0.1;
const _topUpAnycall = async (
  amount: number | string,
  estimateGas: boolean
  ...
  const gas = await anycallContract.deposit.estimateGas(
    curve.constants.ALIASES.voting_escrow_oracle,
...
export const topUpAnycallEstimateGas = async (
  amount: number | string = DEFAULT_AMOUNT
): Promise<number> => {
  return (await _topUpAnycall(amount, true)) as number;
};

export const topUpAnycall = async (
  amount: number | string = DEFAULT_AMOUNT
): Promise<string> => {
  return (await _topUpAnycall(amount, false)) as string;
};

const _sendBlockhash = async (
  block: number,
  chainId: IChainId,
  estimateGas: boolean
): Promise<string | number> => {
  ...
  const veOracleContract =
    curve.contracts[curve.constants.ALIASES.voting_escrow_oracle].contract;
  const gas = await veOracleContract.send_blockhash.estimateGas(
    block,
    chainId,
    curve.constantOptions
  );
  ...
  const gasLimit = mulBy1_3(gas);
  return (
    await veOracleContract.deposit(
      curve.constants.ALIASES.voting_escrow_oracle,
...
export const sendBlockhashEstimateGas = async (
  block: number,
  chainId: IChainId
): Promise<number> => {
...
export const sendBlockhash = async (
  block: number,
  chainId: IChainId
): Promise<string> => {
...
const _submitProof = async (
  ...
  const veOracleContract =
    curve.contracts[curve.constants.ALIASES.voting_escrow_oracle].contract;
  const gas = await veOracleContract.submit_state.estimateGas(
    address,
    "0x" + proof.block_header_rlp,
    "0x" + proof.proof_rlp,
    curve.constantOptions
  );
  if (estimateGas) return Number(gas);
  ...
  const gasLimit = mulBy1_3(gas);
  return (
    await veOracleContract.submit_state(
      address,
      "0x" + proof.block_header_rlp,
      "0x" + proof.proof_rlp,
      { ...curve.options, gasLimit }
...
export const submitProofEstimateGas = async (
  block: number,
  address = curve.signerAddress
): Promise<number> => {
