...
export const BN = (val: number | string): BigNumber =>
  new BigNumber(checkNumber(val));
...
export const mulBy1_3 = (n: bigint): bigint =>
  (n * curve.parseUnits("130", 0)) / curve.parseUnits("100", 0);

// coins can be either addresses or symbols
export const _getCoinAddressesNoCheck = (
  ...coins: string[] | string[][]
): string[] => {
  ...
  return coins
    .map((c) => c.toLowerCase())
    .map((c) => curve.constants.COINS[c] || c);
};
...
export const _getCoinAddresses = (
  ...
  const availableAddresses = [
    ...Object.keys(curve.constants.DECIMALS),
    ...curve.constants.GAUGES,
  ];
...
export const _getBalances = async (
  ...
  for (const address of addresses) {
    balances[address] = _balances[address].map((b, i: number) =>
      curve.formatUnits(b, decimals[i])
    );
  }
...
export const _getAllowance = async (
  ...
  if (_coins.length === 1) {
    allowance = [
      await curve.contracts[_coins[0]].contract.allowance(
        address,
        spender,
    ...
    if (allowance[i] < amounts[i]) {
      ...
      if (allowance[i] > curve.parseUnits("0")) {
        const gasLimit = mulBy1_3(
          await contract.approve.estimateGas(
            spender,
            curve.parseUnits("0"),
            curve.constantOptions
          )
        ...
        txHashes.push(
          (
            await contract.approve(spender, curve.parseUnits("0"), {
              ...curve.options,
              gasLimit,
            })
          ).hash
      ...
      txHashes.push(
        (
          await contract.approve(spender, _approveAmount, {
            ...curve.options,
            gasLimit,
          })
        ).hash
...
export const ensureAllowanceEstimateGas = async (
  ...
  const _amounts = amounts.map((a, i) => parseUnits(a, decimals[i]));
  const address = curve.signerAddress;
  const allowance: bigint[] = await _getAllowance(
    coinAddresses,
    address,
    spender
  );
...
export const getPoolIdBySwapAddress = (swapAddress: string): string => {
  ...
  return Object.entries(poolsData).filter(
    ([_, poolData]) =>
...
const _getTokenAddressBySwapAddress = (swapAddress: string): string => {
  const poolsData = curve.getPoolsData();
  const res = Object.entries(poolsData).filter(
    ([_, poolData]) =>
      poolData.swap_address.toLowerCase() === swapAddress.toLowerCase()
  );
    ...
    for (const pool of extendedPoolData.poolData) {
      if (pool.gaugeAddress) {
        if (!pool.gaugeCrvApy) {
          ...
          apyDict[pool.gaugeAddress.toLowerCase()] = [
            pool.gaugeCrvApy[0] ?? 0,
            pool.gaugeCrvApy[1] ?? 0,
          ];
        }
      }
    }
...
export const _getRewardsFromApi = async (): Promise<
  IDict<IRewardFromApi[]>
> => {
  const network = curve.constants.NETWORK_NAME;
  const allTypesExtendedPoolData = await _getAllPoolsFromApi(network);
...
export const _getUsdRate = async (assetId: string): Promise<number> => {
  if (
    curve.chainId === 1 &&
    assetId.toLowerCase() === "0x8762db106b2c2a0bccb3a80d1ed41273552616e8"
  )
    return 0; // RSR
  ...
  if ((_usdRatesCache[assetId]?.time || 0) + 600000 < Date.now()) {
    ...
    try {
      _usdRatesCache[assetId] = {
        rate: response.data[assetId]["usd"] ?? 0,
        time: Date.now(),
      };
    } catch (err) {
      // TODO pay attention!
...
const _getNetworkName = (
  network: INetworkName | IChainId = curve.chainId
): INetworkName => {
  if (typeof network === "number" && NETWORK_CONSTANTS[network]) {
    return NETWORK_CONSTANTS[network].NAME;
  } else if (
    typeof network === "string" &&
    Object.values(NETWORK_CONSTANTS)
      .map((n) => n.NAME)
      .includes(network)
  ) {
    return network;
...
const _getChainId = (
  ...
  if (typeof network === "number" && NETWORK_CONSTANTS[network]) {
    ...
    const idx = Object.values(NETWORK_CONSTANTS)
      .map((n) => n.NAME)
      .indexOf(network);
...
export const getTVL = async (
  ...
  return allTypesExtendedPoolData.reduce(
    (sum, data) => sum + (data.tvl ?? data.tvlAll ?? 0),
    0
  );
};
...
export const getVolume = async (
  ...
  if (["moonbeam", "kava", "celo", "aurora"].includes(network)) {
    ...
    if (curve.chainId !== chainId)
      throw Error(
        "To get volume for Moonbeam, Kava, Celo or Aurora connect to the network first"
      );
  ...
  const { totalVolume, cryptoVolume, cryptoShare } = await _getSubgraphData(
    network
  );
