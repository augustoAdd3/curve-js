...
export const lowerCasePoolDataAddresses = (
  ...
  for (const poolData of poolsData) {
    ...
    if (poolData.implementationAddress)
      poolData.implementationAddress =
        poolData.implementationAddress.toLowerCase();
...
export async function getFactoryPoolsDataFromApi(
  this: ICurve,
  factoryType:
    | "factory"
    | "factory-crvusd"
  ...
  const isCrypto =
    factoryType === "factory-crypto" || factoryType === "factory-tricrypto";
  let rawPoolList: IPoolDataFromApi[] = lowerCasePoolDataAddresses(
    (await _getPoolsFromApi(network, factoryType)).poolData
  );
  if (!isCrypto) {
    rawPoolList = rawPoolList.filter(
      (p) =>
        p.implementationAddress in
        FACTORY_CONSTANTS[this.chainId].implementationABIDict
    );
  }
  // Filter duplications
  const mainAddresses = Object.values(this.constants.POOLS_DATA).map(
    (pool: IPoolData) => pool.swap_address
  );
  ...
  rawPoolList.forEach((pool) => {
    ...
    if (this.chainId === 137) {
      coinAddresses = coinAddresses.map((a) =>
    ...
    if (isCrypto) {
      const wrappedCoinNames = pool.coins.map((c) =>
        c.symbol === nativeToken.symbol ? nativeToken.wrappedSymbol : c.symbol
      );
      const underlyingCoinNames = pool.coins.map((c) =>
        c.symbol === nativeToken.wrappedSymbol ? nativeToken.symbol : c.symbol
      );
      const underlyingCoinAddresses = coinAddresses.map((addr) =>
        addr === nativeToken.wrappedAddress ? nativeToken.address : addr
      );
      ...
      if (factoryType !== "factory-tricrypto" && basePoolId) {
        // isMeta
        const allPoolsData = {
        ...
        const basePoolCoinNames = [
          ...allPoolsData[basePoolId].underlying_coins,
        ];
        ...
        FACTORY_POOLS_DATA[pool.id] = {
          ...
          gauge_address: pool.gaugeAddress
            ? pool.gaugeAddress
        ...
        FACTORY_POOLS_DATA[pool.id] = {
          name:
            factoryType === "factory-tricrypto"
              ? pool.name
              : pool.name.split(": ")[1].trim(),
          full_name: pool.name,
          symbol: pool.symbol,
          reference_asset: "CRYPTO",
          swap_address: pool.address,
          ...
          swap_abi:
            factoryType === "factory-tricrypto"
              ? tricryptoFactorySwapABI
              : cryptoFactorySwapABI,
          gauge_abi: this.chainId === 1 ? factoryGaugeABI : gaugeChildABI,
          in_api: true,
        };
      }
      ...
      const basePoolIds = Object.values(implementationBasePoolIdDict).filter(
        (poolId, i, arr) => arr.indexOf(poolId) === i
      );
      const allPoolsData = {
        ...this.constants.POOLS_DATA,
        ...FACTORY_POOLS_DATA,
      ...
      const basePoolId =
        implementationBasePoolIdDict[pool.implementationAddress];
