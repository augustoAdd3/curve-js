...
const _deployStablePlainPool = async (
  ...
  if (![0, 1, 2, 3].includes(assetType))
    throw Error(
      "Invalid assetType. Must be one of: 0 = USD, 1 = ETH, 2 = BTC, 3 = Other"
    );
  ...
  const _coins = coins.concat(
    Array(4 - coins.length).fill(curve.constants.ZERO_ADDRESS)
  );
  ...
  const useProxy =
    ...
    (curve.chainId === 1 &&
      ...
      implementationIdx === 5 &&
      emaTime !== 600) ||
    ((curve.chainId === 42161 || curve.chainId == 10) &&
      coins.length === 2 &&
      implementationIdx === 0 &&
      emaTime !== 600);
  const setOracle =
  ...
  const contractAddress =
    useProxy || setOracle
      ? curve.constants.ALIASES.factory_admin
      : curve.constants.ALIASES.factory;
  ...
  if (setOracle) {
    const methodId =
      methodName === "0x00000000"
        ? "0x00000000"
        : ethers.id(methodName).substring(0, 10);
    args.push(methodId, oracleAddress);
  }
  const methodToCall = setOracle
    ? "deploy_plain_pool_and_set_oracle"
    : "deploy_plain_pool";
  const gas = await contract[methodToCall].estimateGas(
    ...args,
...
export const deployStablePlainPoolEstimateGas = async (
  ...
  return (await _deployStablePlainPool(
    name,
    symbol,
    coins,
    A,
...
export const getDeployedStablePlainPoolAddress = async (
...
export const _setOracle = async (
  ...
  const methodId =
    methodName === "0x00000000"
      ? "0x00000000"
      : ethers.id(methodName).substring(0, 10);
  const _gas = await poolContract.set_oracle.estimateGas(
    methodId,
    oracleAddress,
    curve.constantOptions
  );
...
export const setOracleEstimateGas = async (
  poolAddress: string,
  oracleAddress = curve.constants.ZERO_ADDRESS,
  methodName = "0x00000000"
): Promise<number> => {
  return (await _setOracle(
    poolAddress,
    oracleAddress,
    methodName,
    true
...
const _deployStableMetaPool = async (
  ...
  const gas = await contract.deploy_metapool.estimateGas(
    basePool,
    name,
    symbol,
    coin,
  ...
  const gasLimit = mulBy1_3(gas);
  await curve.updateFeeData();
  return await contract.deploy_metapool(
    basePool,
    name,
    symbol,
    coin,
...
export const deployStableMetaPoolEstimateGas = async (
  ...
  return (await _deployStableMetaPool(
    basePool,
    name,
    symbol,
    coin,
...
export const getDeployedStableMetaPoolAddress = async (
  tx: ethers.ContractTransactionResponse
): Promise<string> => {
  const txInfo = await tx.wait();
  if (!txInfo) throw Error("Can't get tx info");
  for (let i = txInfo.logs.length - 1; i > -1; i--) {
    if ("args" in txInfo.logs[i]) {
      const basePoolId = getPoolIdBySwapAddress(
        (txInfo.logs[i] as ethers.EventLog).args[1]
      );
...
const _deployCryptoPool = async (
  ...
  if (BN(outFee).lt(BN(midFee)))
    throw Error(
      `outFee must be >= midFee. Passed outFee = ${outFee} < midFee = ${midFee}`
    );
  if (BN(outFee).gt(100))
    throw Error(`outFee must be <= 100. Passed outFee = ${outFee}`);
  if (BN(allowedExtraProfit).lt(0))
    throw Error(
      `allowedExtraProfit must be >= 0. Passed allowedExtraProfit = ${allowedExtraProfit}`
    );
  if (BN(allowedExtraProfit).gt(0.01))
    throw Error(
      `allowedExtraProfit must be <= 0.01. Passed allowedExtraProfit = ${allowedExtraProfit}`
    );
  if (BN(feeGamma).lt(0))
  ...
  if (BN(adjustmentStep).lt(0))
    throw Error(
      `adjustmentStep must be >= 0. Passed adjustmentStep=${adjustmentStep}`
    );
  if (BN(adjustmentStep).gt(1))
    throw Error(
      `adjustmentStep must be <= 1. Passed adjustmentStep=${adjustmentStep}`
    );
  if (BN(maHalfTime).lt(0))
    throw Error(`maHalfTime must be >= 0. Passed maHalfTime=${maHalfTime}`);
  if (BN(maHalfTime).gt(604800))
    throw Error(
      `maHalfTime must be <= 604800. Passed maHalfTime=${maHalfTime}`
    );
  if (BN(initialPrice).lt(1e-12))
    throw Error(
      `initialPrice must be >= 1e-12. Passed initialPrice=${initialPrice}`
    );
  if (BN(initialPrice).gt(1e12))
    throw Error(
      `initialPrice must be <= 1e12. Passed initialPrice=${initialPrice}`
    );
  ...
  const contract =
    curve.contracts[curve.constants.ALIASES.crypto_factory].contract;
...
export const getDeployedCryptoPoolAddress = async (
  tx: ethers.ContractTransactionResponse
): Promise<string> => {
  const txInfo = await tx.wait();
  if (!txInfo) throw Error("Can't get tx info");
  ...
  return (
    (await contract.minter(curve.constantOptions)) as string
  ).toLowerCase();
};
...
const _deployTricryptoPool = async (
  ...
  if (BN(allowedExtraProfit).gt(1))
    throw Error(
      `allowedExtraProfit must be <= 1. Passed allowedExtraProfit = ${allowedExtraProfit}`
    );
  if (BN(feeGamma).lt(0))
    throw Error(`feeGamma must be >= 0. Passed feeGamma = ${feeGamma}`);
  if (BN(feeGamma).gt(1))
  ...
  if (BN(initialPrices[0]).lt(1e-12))
    throw Error(
      `initialPrices[0] must be >= 1e-12. Passed initialPrices[0]=${initialPrices[0]}`
    );
  if (BN(initialPrices[0]).gt(1e12))
    throw Error(
      `initialPrices[0] must be <= 1e12. Passed initialPrices[0]=${initialPrices[0]}`
    );
  if (BN(initialPrices[1]).lt(1e-12))
    throw Error(
      `initialPrices[1] must be >= 1e-12. Passed initialPrices[1]=${initialPrices[1]}`
    );
  if (BN(initialPrices[1]).gt(1e12))
    throw Error(
      `initialPrices[1] must be <= 1e12. Passed initialPrices[1]=${initialPrices[1]}`
    );
  ...
  const _initialPrices = [
    parseUnits(initialPrices[0]),
    parseUnits(initialPrices[1]),
  ];
  const contract =
    curve.contracts[curve.constants.ALIASES.tricrypto_factory].contract;
...
export const getDeployedTricryptoPoolAddress = async (
  tx: ethers.ContractTransactionResponse
): Promise<string> => {
  const txInfo = await tx.wait();
  if (!txInfo) throw Error("Can't get tx info");
...
const _deployGauge = async (
  pool: string,
  factory: string,
  estimateGas: boolean
): Promise<ethers.ContractTransactionResponse | number> => {
  const contract = curve.contracts[factory].contract;
  const gas = await contract.deploy_gauge.estimateGas(
    pool,
    curve.constantOptions
  );
  if (estimateGas) return Number(gas);
...
export const deployGaugeEstimateGas = async (
  pool: string,
  factory: string
): Promise<number> => (await _deployGauge(pool, factory, true)) as number;

export const deployGauge = async (
  pool: string,
  factory: string
): Promise<ethers.ContractTransactionResponse> =>
  (await _deployGauge(
...
export const getDeployedGaugeAddress = async (
  tx: ethers.ContractTransactionResponse
): Promise<string> => {
  const txInfo = await tx.wait();
  if (!txInfo) throw Error("Can't get tx info");
