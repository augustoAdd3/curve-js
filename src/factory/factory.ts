...
async function getFactoryIdsAndSwapAddresses(
  ...
  const factoryMulticallContract =
    this.contracts[factoryAddress].multicallContract;
  ...
  const prefix =
    factoryAddress === this.constants.ALIASES.factory
      ? "factory-v2-"
      : "factory-crvusd-";
...
> {
  ...
  const referenceAssets = _handleReferenceAssets(
    res.filter((a, i) => i % 7 == 2) as bigint[]
  );
...
async function getCoinsData(
  ...
  const decimals = (res.filter((a, i) => i % 2 == 1) as bigint[]).map((_d) =>
...
export async function getFactoryPoolData(
  ...
  fromIdx = 0,
  swapAddress?: string,
  factoryAddress = curve.constants.ALIASES.factory
