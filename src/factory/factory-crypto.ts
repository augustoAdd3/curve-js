...
async function getRecentlyCreatedCryptoPoolId(
  this: ICurve,
  swapAddress: string
): Promise<string> {
  const factoryContract =
    this.contracts[this.constants.ALIASES.crypto_factory].contract;

  const poolCount = Number(
    curve.formatUnits(await factoryContract.pool_count(this.constantOptions), 0)
  );
...
async function getCryptoFactoryIdsAndSwapAddresses(
  this: ICurve,
  fromIdx = 0
  ...
  const factoryMulticallContract =
    this.contracts[this.constants.ALIASES.crypto_factory].multicallContract;
...
async function getPoolsData(
  ...
  const tokenAddresses = (res.filter((a, i) => i % 3 == 0) as string[]).map(
    (a) => a.toLowerCase()
  );
  const gaugeAddresses = (res.filter((a, i) => i % 3 == 1) as string[]).map(
    (a) => a.toLowerCase()
  );
  const coinAddresses = _handleCoinAddresses.call(
    this,
    res.filter((a, i) => i % 3 == 2) as string[][]
  );

  return [tokenAddresses, gaugeAddresses, coinAddresses];
}

function setCryptoFactorySwapContracts(
  this: ICurve,
  factorySwapAddresses: string[]
): void {
...
function setCryptoFactoryTokenContracts(
  this: ICurve,
  factoryTokenAddresses: string[]
): void {
...
function setCryptoFactoryGaugeContracts(
  this: ICurve,
  ...
  factoryGaugeAddresses
    ...
    .forEach((addr, i) => {
      this.setContract(
        addr,
        this.chainId === 1 ? factoryGaugeABI : gaugeChildABI
      );
    });
}

function setCryptoFactoryCoinsContracts(
  this: ICurve,
  coinAddresses: string[][]
): void {
...
function getCryptoFactoryUnderlyingCoinAddresses(
  this: ICurve,
  coinAddresses: string[][]
): string[][] {
  return [
    ...coinAddresses.map((coins: string[]) =>
      coins.map((c) =>
        c === this.constants.NATIVE_TOKEN.wrappedAddress
...
function getExistingCoinAddressNameDict(this: ICurve): IDict<string> {
  ...
  dict[this.constants.NATIVE_TOKEN.address] =
    this.constants.NATIVE_TOKEN.symbol;
...
async function getCoinsData(
  ...
  const decimals = (res2.filter((a, i) => i % 2 == 1) as bigint[]).map((_d) =>
    Number(curve.formatUnits(_d, 0))
  );
  ...
  coinAddrNamesDict[this.constants.NATIVE_TOKEN.address] =
    this.constants.NATIVE_TOKEN.symbol;
...
export async function getCryptoFactoryPoolData(
  this: ICurve,
  fromIdx = 0,
  swapAddress?: string
  ...
  const [poolIds, swapAddresses] = swapAddress
    ? [
        [await getRecentlyCreatedCryptoPoolId.call(this, swapAddress)],
        [swapAddress.toLowerCase()],
      ]
    : await getCryptoFactoryIdsAndSwapAddresses.call(this, fromIdx);
  if (poolIds.length === 0) return {};

  const [tokenAddresses, gaugeAddresses, coinAddresses] =
    await getPoolsData.call(this, swapAddresses);
  ...
  const underlyingCoinAddresses = getCryptoFactoryUnderlyingCoinAddresses.call(
    this,
    coinAddresses
  );
  ...
  for (let i = 0; i < poolIds.length; i++) {
    const lpTokenBasePoolIdDict =
      CRYPTO_FACTORY_CONSTANTS[this.chainId].lpTokenBasePoolIdDict;
    const basePoolIdZapDict =
      CRYPTO_FACTORY_CONSTANTS[this.chainId].basePoolIdZapDict;
    ...
    if (basePoolId) {
      ...
      const basePoolCoinAddresses = [
        ...allPoolsData[basePoolId].underlying_coin_addresses,
      ];
      const basePoolDecimals = [
        ...allPoolsData[basePoolId].underlying_decimals,
      ];
      ...
      CRYPTO_FACTORY_POOLS_DATA[poolIds[i]] = {
        ...
        underlying_coins: [
          coinAddressNameDict[underlyingCoinAddresses[i][0]],
          ...basePoolCoinNames,
        ],
        wrapped_coins: [
          ...coinAddresses[i].map((addr) => coinAddressNameDict[addr]),
        ],
        underlying_coin_addresses: [
          underlyingCoinAddresses[i][0],
          ...basePoolCoinAddresses,
        ],
        wrapped_coin_addresses: coinAddresses[i],
        underlying_decimals: [
          coinAddressDecimalsDict[underlyingCoinAddresses[i][0]],
          ...basePoolDecimals,
        ],
        wrapped_decimals: [
          ...coinAddresses[i].map((addr) => coinAddressDecimalsDict[addr]),
        ],
      ...
      CRYPTO_FACTORY_POOLS_DATA[poolIds[i]] = {
        ...
        is_crypto: true,
        is_plain:
          underlyingCoinAddresses[i].toString() === coinAddresses[i].toString(), // WETH/ETH - NOT Plain
        is_factory: true,
        underlying_coins: [
          ...underlyingCoinAddresses[i].map(
