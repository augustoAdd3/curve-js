...
async function getRecentlyCreatedCryptoPoolId(
  ...
  const factoryContract =
    this.contracts[this.constants.ALIASES.tricrypto_factory].contract;
...
async function getCryptoFactoryIdsAndSwapAddresses(
  ...
  const factoryMulticallContract =
    this.contracts[this.constants.ALIASES.tricrypto_factory].multicallContract;
  ...
  let factories: { id: string; address: string }[] = (
    ...
    id: `factory-tricrypto-${fromIdx + i}`,
...
async function getPoolsData(
  ...
  const gaugeAddresses = (res.filter((a, i) => i % 2 == 0) as string[]).map(
    (a) => a.toLowerCase()
  );
  const coinAddresses = _handleCoinAddresses.call(
    this,
    res.filter((a, i) => i % 2 == 1) as string[][]
  );
...
export async function getTricryptoFactoryPoolData(
  this: ICurve,
  fromIdx = 0,
  swapAddress?: string
  ...
  const [gaugeAddresses, coinAddresses] = await getPoolsData.call(
    this,
    swapAddresses
  );
  ...
  for (let i = 0; i < poolIds.length; i++) {
    TRICRYPTO_FACTORY_POOLS_DATA[poolIds[i]] = {
      ...
      is_factory: true,
      underlying_coins: [
        ...underlyingCoinAddresses[i].map((addr) => coinAddressNameDict[addr]),
      ],
      wrapped_coins: [
