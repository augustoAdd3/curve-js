...
`npm install @augustoadd3/curveapi`

## Init

```ts
import curve from "@augustoadd3/curveapi";
...
(async () => {
  ...
  const balances2 = await curve.getBalances(
    ["aDAI", "aSUSD"],
  ...
  await curve.getAllowance(
    ["DAI", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"],
    curve.signerAddress,
    spender
  );
  // [ '0.0', '0.0' ]
  await curve.hasAllowance(
    ["DAI", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"],
    ["1000", "1000"],
    curve.signerAddress,
    spender
...
(async () => {
  ...
  const wrappedExpected = await pool.withdrawOneCoinWrappedExpected(
    "10",
    "MIM"
  );
...
(async () => {
  ...
  const wrappedPriceImpact = await pool.swapWrappedPriceImpact(
    "3crv",
    "MIM",
    10
  );
...
(async () => {
  ...
  const { unlockTime: currentUnlockTime } =
    await curve.boosting.getLockedAmountAndUnlockTime();
...
(async () => {
  ...
  await curve.boosting.sidechain.checkBlockhash(17377005);
  // true
  await curve.boosting.sidechain.submitProof(
    17377005,
...
(async () => {
  ...
  const spender = "0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7"; // 3pool swap address
  await curve.estimateGas.ensureAllowance(
    ["DAI", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"],
    [1000, 1000],
    spender
  );
...
(async () => {
  ...
  // }
  const poolAddress = await curve.factory.getDeployedPlainPoolAddress(
    deployPoolTx
  );
  ...
  //     ...
  // }
  const gaugeAddress = await curve.factory.getDeployedGaugeAddress(
    deployGaugeTx
  );
...
(async () => {
  ...
  //     ...
  // }
  const poolAddress = await curve.factory.getDeployedMetaPoolAddress(
    deployPoolTx
  );
...
(async () => {
  ...
  //     ...
  // }
  const poolAddress = await curve.cryptoFactory.getDeployedPoolAddress(
    deployPoolTx
  );
  ...
  const poolId = await curve.cryptoFactory.fetchRecentlyDeployedPool(
    poolAddress
  );
  // factory-crypto-155
  const pool = curve.getPool(poolId);
...
(async () => {
  ...
  //     ...
  // }
  const poolAddress = await curve.tricryptoFactory.getDeployedPoolAddress(
    deployPoolTx
  );
  ...
  const poolId = await curve.tricryptoFactory.fetchRecentlyDeployedPool(
    poolAddress
  );
  // factory-tricrypto-2
  const pool = curve.getPool(poolId);
